...
import { Dropdown, Button, MenuProps, Modal, Tooltip } from 'antd';
import { MoreOutlined, UnorderedListOutlined, EditOutlined, CloudSyncOutlined, ProfileOutlined, DatabaseOutlined } from '@ant-design/icons';
...
import { getDuratinon, formatTimestamp2 } from '@/utils/format';
...
const DeviceInfoTable: React.FC<DeviceInfoTableProps> = ({
  ...
  const handleRequest = async (params: {
    ...
    searchItems = updateSearchItemWithValue(searchItems, "active_status", params.ActiveStatus)
  ...
  const columns: ProColumns[] = [
    ...
    {
      title: 'RF Status',
      dataIndex: ['Properties', 'RFTxStatus'],
      key: 'RFTxStatus',
      search: false,
      valueType: 'select',
      valueEnum: {
    ...
    },
    {
      ...
      search: false,
      render: (text: any, record: API.Device) => {
        const data = record.Properties as unknown as { MMEIp1?: string, MMEIp2?: string };
        return (
          <>
            <Tooltip title={'MME1 IP: ' + data.MMEIp1}>
              <DatabaseOutlined color={data.MMEIp1 && data.MMEIp1 !== '-' ? "#52c41a" : "#eb2f96"} />
            </Tooltip>
            <Tooltip title={'MME2 IP: ' + data.MMEIp2}>
              <DatabaseOutlined color={data.MMEIp2 && data.MMEIp2 !== '-' ? "#52c41a" : "#eb2f96"} />
            </Tooltip>
          </>
        )
      },
